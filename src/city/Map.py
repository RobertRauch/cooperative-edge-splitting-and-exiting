...
def loadDriveGraphFromFile(filepath_pickle: str):
    """
    loads drive graph
    :param filename: filepath of given file
    :return: osmnx drive graph
    """
    if path.exists(filepath_pickle):
        print(f"Drive graph will be loaded from cache file: {filepath_pickle}")
        with open(filepath_pickle, 'rb') as inp:
            return pickle.load(inp)
    else:
        raise FileNotFoundError(f"File {filepath_pickle} doesn't exist.")


class Map:

    def __init__(self, radius, location: Location, oneWayEnabled=False, removeDeadends=True, driveGraph=None, useCache=True):
        """
    ...
    @classmethod
    def fromFile(cls, filepath_pickle: str, radius: float, contains_deadends: bool) -> "Map":
        driveGraph = loadDriveGraphFromFile(filepath_pickle)
        return cls.fromGraph(driveGraph, radius, contains_deadends)

    @classmethod
    def fromGraph(cls, driveGraph, radius: float, contains_deadends: bool) -> "Map":
        return cls(
            radius=radius,
            location=Location(40,40), # dummy is needed
            removeDeadends=not contains_deadends,
    ...
    def plotRoute(self, route, name):
        fig, ax = ox.plot_graph_route(self.driveGraph, route, node_size=1.5, edge_linewidth=0.9, orig_dest_node_size=2,
                                      save=True, show=False, route_linewidth=1.8, file_format='pdf',
                                      filename=name)
    ...
    def loadDriveGraphFromCache(self, filename):
        ...
        :return: osmnx drive graph
        """
        cache_dirpath = "iism_cache/driveGraphCache"
        pathname = f"{cache_dirpath}/{filename}.pkl"
        try:
            return loadDriveGraphFromFile(pathname)
        except FileNotFoundError as e:
            print(f"Cached file {filename} not found.")
            return None
