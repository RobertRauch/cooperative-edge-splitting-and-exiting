...
from . import dict_utils, json_utils
from .cli import parse_args
from .file_utils import FileExistsException, throw_or_remove_file
...
class BaseSimulationConfig(BaseModel):
    ...
    comment: str
    repeat: int
    range_params: Optional[dict] = None
...
def parallel_simulation_run(main_fun: Callable[[Dict[str, any]], None]) -> None:
    ...
    if os.path.isdir(args.config_file):
        ...
        config_dicts = preprocess_config(json_utils.load_dict(args.config_file))
    ...
    try:
        ...
        print(
            f'Directory "{e.path}" already exists. '
    ...
    if len(config_dicts) == 1 or processes == 1:
...
def _handle_repeat(dict_: dict, repeat: int) -> List[dict]:
...
def _rename_by_repeat(dict_: dict, index: int) -> None:
