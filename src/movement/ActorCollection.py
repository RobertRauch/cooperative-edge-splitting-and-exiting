...
from src.movement.movementStrategies.MovementStrategyFactory import MovementStrategyFactory, MovementStrategyFactoryFunctionType
from src.movement.movementStrategies.MovementStrategyType import MovementStrategyType
from src.placeable.Placeable import PlaceableFactory
...
class ActorCollection:
    ...
    def __init__(
        ...
        map,
        ableOfMovement,
        movementStrategy: Union[MovementStrategyType, MovementStrategyFactoryFunctionType],
        mapGrid,
        secondsPerTick,
    ) -> None:
        self.name = name
        ...
        if isinstance(movementStrategy, MovementStrategyType):
            self.movementStrategy = MovementStrategyFactory().getStrategy(
                movementStrategy, self.locationsTable, self.actorSet, self.map, self.mapGrid
            )
        else:
    ...
    def addPlaceableFactory(self, factory: PlaceableFactory, count: int) -> 'ActorCollection':
        # NOTE Added
        for _ in range(count):
            placeable = factory.createPlaceable(self.locationsTable, self.map)
            self.actorSet[placeable.id] = placeable
            placeable.tableRow = self.locationsTable.insertNewActor(placeable)

        return self

    def generateMovables(self, count: int) -> List[Movable]:
        # NOTE Added
        movables = []
        for _ in range(count):
            m = Movable(self.locationsTable, self.map)
            self.actorSet[m.id] = m
            m.tableRow = self.locationsTable.insertNewActor(m)

            location = self.map.getRandomIntersectionNode()
            location.setGridCoordinates(*self.mapGrid.getGridCoordinates(location))
            m.setTargetReached(True)
            m.setTargetLocation(location)
            m.setLocation(self.map.getRandomIntersectionNode())
